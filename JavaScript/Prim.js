    ...
    for(let i = 0; i < primGraph.vertexNum; i++){
        if(dist[i] < INF){
            ...
            l.Inite_Point(primGraph.vertexs[i].PosX, primGraph.vertexs[i].PosY,
                primGraph.vertexs[dist[i]].PosX, primGraph.vertexs[dist[i]].PosY);
            l.DrawLine(ctx, "purple", 3);
        }
    }
}
...
function drawAll(ctx) {
    clearAll(ctx);
    primGraph.DrawGraph(ctx, "red", 3, "blue", 3);
    drawPath(ctx);
    for(let i = 0; i < primGraph.vertexNum; i++){
    ...
    graphTable.DrawTable(ctx, "black", 2, "red", MICROSOFTFONT);
    distTable.DrawTable(ctx, "black", 2, "blue", MICROSOFTFONT);
    visTable.DrawTable(ctx, "black", 2, "green", MICROSOFTFONT);
}
...
findInDistance = function(ctx, j){
    return new Promise((resolve, reject) => {
        distTable.DrawTableElement(ctx, 0, j, "yellow", 3, "black", MICROSOFTFONT);
        setTimeout(()=>{
            drawAll(ctx);
...
findInVisited = function(ctx, j){
    return new Promise((resolve, reject) => {
        visTable.DrawTableElement(ctx, 0, j, "yellow", 3, "black", MICROSOFTFONT);
        setTimeout(()=>{
            drawAll(ctx);
            resolve("VisitedFind");
        }, 30*TIMEINTERVAL);
...
drawNode = function(ctx, i){
    return new Promise((resolve, reject) => {
        primGraph.vertexs[i].DrawHollowCircle(ctx, "yellow", 3);
        setTimeout(()=>{
            drawAll(ctx);
            resolve("drawNode");
        }, 30*TIMEINTERVAL);
    ...
    return new Promise(async (resolve, reject) => {
        ...
        for(let i = 0; i < primGraph.vertexNum; i++){
            ...
            if(!vis[i]){
                ...
                if(primGraph.graph[i][dist[i]] < Min){
                    Min = primGraph.graph[i][dist[i]];
                    pos = i;
        ...
        //primGraph.vertexs[pos].DrawHollowCircle(ctx, "purple", 3);
        resolve(pos);
    })
};
...
updateDist = async function(ctx, pos){
    ...
    for(let i = 0; i < primGraph.vertexNum; i++){
        await findInVisited(ctx, i);
        if(!vis[i]){
            await findEdgeInMatrix(ctx, i, pos);
            if(primGraph.graph[pos][i] < dist[i]){
